...
def carregar_os_gerente(gerente):
    try:
        ...
        logger.error(f"Erro ao carregar OS para gerente {gerente}: {e}")
        return []
...
def carregar_os_prestadores():
    try:
        ...
        logger.error(f"Erro ao carregar OS de prestadores: {e}")
        return []
...
def login():
    try:
        ...
        logger.error(f"Erro na rota /login para usuário {username}: {e}")
        flash('Erro ao processar o login. Tente novamente.', 'danger')
        return render_template('login.html')
...
def painel():
    try:
        ...
        logger.error(f"Erro na rota /painel para gerente {session.get('gerente')}: {e}")
        flash('Erro ao carregar o painel.', 'danger')
        return redirect(url_for('login'))
...
def upload_profile_picture():
    try:
        ...
        logger.error(f"Erro na rota /upload_profile_picture para usuário {username}: {e}")
        flash('Erro ao processar o upload da foto.', 'danger')
        return redirect(url_for('painel_manutencao' if 'manutencao' in session else 'painel' if 'gerente' in session else 'login'))
...
def painel_manutencao():
    try:
        ...
        logger.error(f"Erro na rota /painel_manutencao para usuário {session.get('manutencao')}: {e}")
        flash('Erro ao carregar o painel de manutenção.', 'danger')
        return redirect(url_for('login'))
...
def atribuir_prestador(os_numero):
    try:
        ...
        logger.error(f"Erro na rota /atribuir_prestador para OS {os_numero}: {e}")
        flash('Erro ao atribuir o prestador.', 'danger')
        return redirect(url_for('painel_manutencao'))
...
def admin_panel():
    try:
        ...
        logger.error(f"Erro na rota /admin: {e}")
...
def logout():
    try:
        ...
        logger.error(f"Erro na rota /logout: {e}")
        flash('Erro ao desconectar. Sessão limpa.', 'warning')
        session.clear()
        return redirect(url_for('login'))

...
db = SQLAlchemy(app)  # Inicializa db antes dos modelos
...
# --- Função para remover OS em todos os JSONs ---
...
def carregar_os_prestadores():
    ...
    for dados_prestador in lista_prestadores:
        ...
        if not username_prestador or dados_prestador.get('tipo') == 'manutencao':
            continue
...
def upload_profile_picture():
    ...
    if not user_db_entry:
        ...
        if 'manutencao' in session:
            redirect_target = 'painel_manutencao'
        elif 'gerente' in session:
            redirect_target = 'painel'
                    ...
                    if data_os_str_prest:
                        ...
                        for fmt_prest in ("%d/%m/%Y", "%Y-%m-%d", "%d-%m-%y", "%Y/%m/%d"):
                            try:
                                ...
                                continue
                        if data_abertura_obj_prest:
                            dias_abertos_calc_prest = (data_hoje_prest - data_abertura_obj_prest).days
                        else:
                            logger.warning(f"Parse de data falhou para OS {item_proc_prest.get('os','?')} data: '{data_os_str_prest}'")
...
def finalizar_os(os_numero_str):
    ...
    if not data_finalizacao_form or not hora_finalizacao_form:
        ...
        try:
            ...
            for fmt_finalizacao in ("%Y-%m-%d", "%d/%m/%Y"):
                try:
        ...
        if not os_alvo_para_atribuicao:
            ...
            if not dados_prestador_destino or not dados_prestador_destino.get('arquivo_os'):
                ...
                os_formatada_para_prestador = {
                    ...
                    "data": os_alvo_para_atribuicao.get('data_entrada'),
                    "Data": os_alvo_para_atribuicao.get('data_entrada'),
...
def admin_panel():
    ...
    for f_grafico in todas_finalizadas_no_periodo_grafico:
        ...
        chave_periodo_g = ""
        if periodo == 'anual':
            chave_periodo_g = data_finalizacao_obj_g.strftime('%Y-%m')
        elif periodo == 'mensal':
            chave_periodo_g = data_finalizacao_obj_g.strftime('%d/%m')
        elif periodo == 'semanal':
            chave_periodo_g = data_finalizacao_obj_g.strftime('%d/%m')
        elif periodo == 'diario':
...
def exportar_os_finalizadas():
    ...
    if data_inicio_export_str and data_fim_export_str:
        ...
        if periodo_export == 'diario':
            ...
            if inicio_export.month == 12:
                fim_export = inicio_export.replace(year=inicio_export.year + 1, month=1, day=1) - timedelta(microseconds=1)
            else:
                fim_export = inicio_export.replace(month=inicio_export.month + 1, day=1) - timedelta(microseconds=1)
        ...
        if not (inicio_export and fim_export):
            inicio_export, fim_export = None, None
        else:
            query_export = query_export.filter(Finalizacao.registrado_em.between(inicio_export, fim_export))
    ...
    if data_inicio_export_str and data_fim_export_str:
        try:
            titulo_relatorio_periodo = f"{parse(data_inicio_export_str).strftime('%d/%m/%Y')} a {parse(data_fim_export_str).strftime('%d/%m/%Y')}"
        except ValueError:
            titulo_relatorio_periodo = "Período Personalizado"
    ...
    for os_finalizada_item in lista_finalizadas_para_export:
        if pos_y_linha < 60:
            ...
            canvas_pdf.setFont("Helvetica", 8)
        pos_x_col = 40
        canvas_pdf.drawString(pos_x_col, pos_y_linha, os_finalizada_item.os_numero)
        pos_x_col += 50
        canvas_pdf.drawString(pos_x_col, pos_y_linha, os_finalizada_item.gerente)
        pos_x_col += 100
        canvas_pdf.drawString(pos_x_col, pos_y_linha, os_finalizada_item.data_fin)
        pos_x_col += 60
        canvas_pdf.drawString(pos_x_col, pos_y_linha, os_finalizada_item.hora_fin)
        pos_x_col += 50
        observacoes_trunc = (os_finalizada_item.observacoes[:40] + '...') if os_finalizada_item.observacoes and len(os_finalizada_item.observacoes) > 40 else (os_finalizada_item.observacoes or '')
        canvas_pdf.drawString(pos_x_col, pos_y_linha, observacoes_trunc)
        pos_x_col += 180
        canvas_pdf.drawString(pos_x_col, pos_y_linha, format_datetime(os_finalizada_item.registrado_em))
        pos_y_linha -= 15
    canvas_pdf.save()
    try:
        return send_file(caminho_pdf_salvo, as_attachment=True, download_name=nome_arquivo_pdf)
    except Exception as e:
        logger.error(f"Erro ao enviar PDF: {e}")
        flash('Erro ao gerar o PDF para download.', 'danger')
        return redirect(url_for('admin_panel', periodo=periodo_export, data_inicio=data_inicio_export_str, data_fim=data_fim_export_str))
    finally:
        try:
            os.remove(caminho_pdf_salvo)
        except Exception as e:
            logger.warning(f"Erro ao remover PDF temporário {caminho_pdf_salvo}: {e}")
...
def frota_leve():
    ...
    return render_template('frota_leve.html', dados=dados, usuario=usuario)
...
def nova_manutencao_frota_leve():
    ...
    return render_template('nova_frota_leve.html',
                         usuario=session.get('gerente') or session.get('manutencao') or session.get('prestador'))

@app.route('/frota-leve/finalizar/<int:id>', methods=['POST'])
def finalizar_manutencao_frota_leve(id):
    if not session.get('is_admin'):
        return redirect(url_for('login'))
    os_frota = FrotaLeve.query.get_or_404(id)
    os_frota.situacao = 'Finalizado'
    os_frota.saida = request.form['data_fim']
    os_frota.hora_fim = request.form['hora_fim']
    os_frota.obs = (os_frota.obs or '') + '\nFinalização: ' + request.form.get('obs_fim', '')
    db.session.commit()
    return redirect(url_for('frota_leve'))
